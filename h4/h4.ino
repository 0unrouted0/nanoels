...
long surfaceSpeedMPerMin = SURFACE_SPEED_M_PER_MIN; // Constant surface speed target, 0 = off
  ...
  if (lcdHashLine3 != newHashLine3) {
    ...
    if (charIndex > 0) {
      ...
      if (surfaceSpeedMPerMin > 0) {
        ...
        long target = cssTargetRpm(diameterDu, surfaceSpeedMPerMin);
...
void handleStatusJson() {
  ...
  // share with everything else.
  {
    ...
    out += ",\"targetRpm\":" + String(surfaceSpeedMPerMin > 0
        ? cssCappedRpm(cssTargetRpm(diameterDu, surfaceSpeedMPerMin), spindleMaxRpm) : 0);
  }
